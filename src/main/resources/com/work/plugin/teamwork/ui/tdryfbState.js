...
function getBmryfbData(){
    ...
    $.ajax({
        type: "get",
        url: AJS.contextPath() + "/rest/wk-teamwork/1/bmryfbState/tdryfb",
                ...
                treeTR += "<tr id='"+i+"' data-tt-id='"+i+"'>" ;
                    for(var x=0;x<arrColumn.length;x++){
                        ...
                        if(x == 0){
                            ...
                            treeTR +="<td style=\"text-align:center;\"><input type='button' class='Tdetail' style='display: none' onclick='addRow(this,"+data[i].type+","+data[i].pid+")' id=\""+ i +"\" value='"+AJS.I18n.getText('workorg.property.bmryfb.tj_Details')+ "'/></td>";
                        } else {
    ...
    for(var j=0;j< w.length;j++){
            ...
            for(var m=0;m<arrColumn.length;m++){
                ...
                if(m == 0){
                    ...
                    treeTR +="<td style=\"text-align:center;\"><input type='button' class='Tdetail' style='display: none' onclick='addRow(this,"+w[j].type+","+w[j].pid+")' id=\""+ bb+'-'+j +"\" value='"+AJS.I18n.getText('workorg.property.bmryfb.mx_Details')+ "'/></td>";
                } else {
...
function showDetail(but,type,pid){
    $.ajax({
        type: "get",
        url: AJS.contextPath() + "/rest/wk-teamwork/1/bmryfbState/?a=tdryfb&b="+type+"&c="+pid,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var trstr ="";
...
function controlePower() {
    $.ajax({
        ...
        cache: false,
        async:false,
        url: contextPath + "/rest/wk-teamwork/1/bmryfbState/getPower",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success:function (data) {
            for (var k in data) {
